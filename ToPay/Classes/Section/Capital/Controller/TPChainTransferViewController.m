...
//    3 ETH
//    4 BTC
    NSString *address = self.textArray[0].comTextField.text;
    if( ![JudegeCenter isVaildAddrWithTokenId:self.assetModel.tokenId addr:self.textArray[0].comTextField.text] && ![JudegeCenter isAvailableEmail:address] ) {
    ...
    [transView.pasView setEndEditBlock:^(NSString *text)
    {
        if (text.length == 6)
        {
            ...
            [[WYNetworkManager sharedManager] sendPostRequest:WYJSONRequestSerializer url:@"asset/transaction" parameters:@{
                            ...
                            @"password":[QuickGet encryptPwd:text salt:nil],
